...
public class NetworkControlX extends NetworkDirectional {
    ...
    private void tryBreakBlock(@Nonnull BlockMenu blockMenu) {
        ...
        // --- BEGIN FIX ---
        // Check if the block is an InventoryHolder (like a Chest, Decorated Pot, etc.)
        // We must get the state *synchronously* before adding items or scheduling the task.
        if (targetBlock.getState() instanceof InventoryHolder) {
            // Do not break blocks with inventories. This prevents the dupe.
            return;
        }
        // --- END FIX ---
        ...
        final UUID uuid = getOwnerUuid(blockMenu.getLocation());
        if (uuid == null) {
            return;
        }
        ...
        if (resultStack.getAmount() == 0) {
            ...
            FoliaSupport.executeRegion(Networks.getInstance(), targetBlock.getLocation(), () -> {
                ...
                // Check if the block is still the same material before breaking
                if (targetBlock.getType() == material) {
                    ...
                    // Block changed type, refund the item
                    definition.getNode().getRoot().addItemStack(new ItemStack(material, 1));
                    this.blockCache.remove(targetPosition);
                }
            });
