...
public class NetworkVacuum extends NetworkObject {
        ...
        addItemHandler(
                new BlockTicker() {
                    ...
                    public boolean isSynchronized() {
                        // MODIFICATION: Set to true to prevent race conditions
                        // This forces tryAddItem and findItem to run on the main thread
                        return true;
                    }
    ...
    private void findItem(@Nonnull BlockMenu blockMenu) {
        for (int inputSlot : INPUT_SLOTS) {
            ...
            if (inSlot == null || inSlot.getType().isAir()) {
                ...
                // MODIFICATION: Find the first valid item that isn't already marked for no-pickup
                Optional<Item> targetItem = items.stream()
                        .map(Item.class::cast)
                        .filter(item -> !item.isDead() && item.getPickupDelay() <= 0 && !SlimefunUtils.hasNoPickupFlag(item))
                        .findFirst();

                if (targetItem.isEmpty()) {
                    return; // No valid items to pick up
                }
    ...
    private void tryAddItem(@Nonnull BlockMenu blockMenu) {
        ...
        for (int inputSlot : INPUT_SLOTS) {
            ...
            // This method mutates the itemStack, setting its amount to 0 as it's added to the network
